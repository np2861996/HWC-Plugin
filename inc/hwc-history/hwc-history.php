...
function hwc_create_history_page_with_acf_fields()
{
    ...
    // Check if the history page creation has already been run
    $history_page_created = get_option('hwc_history_page_created');
    ...
    if (!$history_page_created) {
        ...
        // Set the flag to indicate the history page was created
        update_option('hwc_history_page_created', true);
    } else {
        // If the history page creation was already done, just get its ID
        $hwc_history_page = get_page_by_path($hwc_history_page_slug);
        if ($hwc_history_page) {
            $hwc_history_page_id = $hwc_history_page->ID;
        }
    ...
    ----------------------------------------------------------------*/
    if (get_page_by_path($hwc_history_page_slug)) {
